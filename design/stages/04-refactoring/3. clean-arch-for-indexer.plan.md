...
Keep `DevContextMcp.Indexer` as the inner indexing feature library containing
...
Create `DevContextMcp.Indexer.Cli` as the one-shot console composition root.
...
- Change `DevContextMcp.Indexer` from an executable to a class library.
...
- Keep use-case ports such as `IIndexStore`, `IPackageSourceClient`, and
  `IPackageProcessor` in `DevContextMcp.Indexer.Abstractions`.
...
- Add `DevContextMcp.Indexer.Cli` as the one-shot console executable.
...
- Add `DevContextMcp.Indexer.Cli` to the solution.
...
- `DevContextMcp.Indexer` changes from an executable to a class library.
- The indexing command moves to `DevContextMcp.Indexer.Cli`.
...
- Update README, architecture, specification, test plans, commands, and
  configuration paths for `DevContextMcp.Indexer.Cli`.
...
- `DevContextMcp.Indexer` is the inner feature boundary and does not reference
  Infrastructure or hosting packages.
- `DevContextMcp.Indexer.Cli` is the only indexing composition root.
