...
internal sealed class LocalDependencyCheck : ILocalDependencyCheck
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            new LocalDependencyCheckResult(true, "database_path_resolves", $"Database path resolves to '{resolvedDatabasePath}'.")
        };

        if (string.IsNullOrWhiteSpace(parent))
        {
            results.Add(new LocalDependencyCheckResult(false, "database_parent_resolves", "Database path does not have a parent directory."));
        }
        else
        {
            _ = Path.GetFullPath(parent);
            results.Add(new LocalDependencyCheckResult(true, "database_parent_resolves", $"Database parent resolves to '{parent}'."));
        }
